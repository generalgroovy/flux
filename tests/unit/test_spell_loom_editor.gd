...
func _test_canonical_slot_weaving() -> void:
	...
	equal(Array(state.spell_wire_ids), [101, 110, 140, 141, 144, 142, 143, 0, 0, 0, 0, 0], "champion kit leads the stable global spell library")
	...
	equal(Array(state.spell_wire_ids), [101, 110, 140, 141, 144, 142, 0, 0, 0, 0, 0, 143], "moving a global spell swaps with the selected position")
	...
	equal(Array(state.spell_wire_ids), [101, 143, 140, 141, 144, 142, 0, 0, 0, 0, 0, 110], "occupied weave preserves every proven spell exactly once")
...
func _test_editor_navigation_and_encoding() -> void:
	...
	equal(editor.selected_wire_id(), CombatTuning.POCKET_ECLIPSE_WIRE_ID, "wrapped selection resolves through stable runtime order")
	equal(editor.request_value(), 535, "Alt+4 global spell has a bounded request encoding")
	equal(SpellLoomEditor.decode_slot_index(535), 11, "request decodes position deterministically")
	equal(SpellLoomEditor.decode_library_index(535), 6, "request decodes the global library index deterministically")
	...
	equal(state.spell_wire_id(12), CombatTuning.POCKET_ECLIPSE_WIRE_ID, "selected global spell occupies Alt+4")
...
func _test_three_spell_kit_weaving() -> void:
	...
	equal(Array(state.spell_wire_ids), [140, 141, 144, 101, 110, 142, 143, 0, 0, 0, 0, 0], "three champion spells lead the stable global library")
	...
	equal(Array(state.spell_wire_ids), [140, 141, 0, 101, 110, 142, 143, 0, 0, 0, 0, 144], "third spell weaving preserves all global spells and honest empty positions")
	var editor := SpellLoomEditor.new()
	editor.open_editor(state)
	equal(editor.available_wire_ids.size(), 7, "Oh Tipi can weave every proven global spell")
	editor.move_selection(-1, -1)
	equal(editor.selected_wire_id(), CombatTuning.POCKET_ECLIPSE_WIRE_ID, "global navigation is independent of champion kit")
	equal(editor.request_value(), 535, "global weave uses the bounded catalog lane")
	equal(SpellLoomEditor.decode_slot_index(535), 11, "global request value decodes Alt+4")
	equal(SpellLoomEditor.decode_library_index(535), 6, "global request value decodes the final proven spell")
