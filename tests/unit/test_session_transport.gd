...
func _test_validation_fails_closed() -> void:
	...
	check(SessionTransport._valid_request_packet({"sequence": 3, "action": SessionTransport.REQUEST_SPELL_EQUIP, "value": 36}), "typed final Spell Loom request validates")
...
func _test_enet_loopback_handshake_and_input() -> void:
	...
	check(client.send_request(6, SessionTransport.REQUEST_SPELL_EQUIP, 36), "client sends a bounded Spell Loom weave")
	...
	equal(int(spell_requests[0].get("value", 0)) if not spell_requests.is_empty() else 0, 36, "host receives the bounded slot/role value")
	check(not client.send_request(7, SessionTransport.REQUEST_SPELL_EQUIP, 37), "out-of-range Spell Loom value fails before transport")
