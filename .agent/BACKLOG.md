...
| 6 | Friend-play vertical slice (shared interactions live) | Stable actors, host input, 60 Hz movement/combat/target snapshots, host-authorized emote/Bell/Loom requests and Windows localhost pass; next: prediction/reconciliation, reconnect and Linux direct-IP smoke |
| 7 | Responsive remote traveller | Predict only the local guest from bounded sent inputs, reconcile to host snapshots without outcome authority, expose latency/correction diagnostics and prove deterministic convergence under delay |
