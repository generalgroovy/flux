...
| --- | --- | --- |
| Get the game | One `FLUX.exe` verifies, installs or updates, repairs, and starts a versioned per-user build without admin or developer tools | Signing, clean uninstall UI, public update discovery and two-PC release proof remain gated |
...
| ---: | --- | --- |
| 1 | One-file Windows lifecycle — complete | Template identity, runtime-only export, checksums, atomic version switch, packaged boot/close, no stray process |
