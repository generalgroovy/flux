...
func _test_catalog() -> void:
	...
	equal(int(first.ability("vector-lance")["flux_cost"]) * 1000, int(CombatTuning.cast_definition(CombatTuning.ACTIVE_1_WIRE_ID)["flux_cost"]), "compiled active Flux cost matches catalog")
	equal(int(first.ability("vector-lance")["startup_ms"]), int(CombatTuning.cast_definition(CombatTuning.ACTIVE_1_WIRE_ID)["startup_ms"]), "compiled active startup matches catalog")
	equal(int(first.ability("rillshot")["wire_id"]), CombatTuning.RILLSHOT_WIRE_ID, "Rillshot wire matches compiled Oh Tipi kit")
	equal(int(first.ability("rillshot")["flux_cost"]) * 1000, int(CombatTuning.cast_definition(CombatTuning.RILLSHOT_WIRE_ID)["flux_cost"]), "Oh Tipi primary has exact positive Flux cost")
	...
	equal(int(first.ability("cinderbolt")["flux_cost"]) * 1000, int(CombatTuning.cast_definition(CombatTuning.CINDERBOLT_WIRE_ID)["flux_cost"]), "Red Baron primary has exact positive Flux cost")
	equal(int(first.ability("cinderbolt")["cooldown_ms"]), int(CombatTuning.cast_definition(CombatTuning.CINDERBOLT_WIRE_ID)["cooldown_ms"]), "Cinderbolt cadence matches compiled behavior")
	...
	equal(content_fan_angles, CombatTuning.cast_definition(CombatTuning.CINDERFAN_WIRE_ID)["projectile_angles_degrees"], "Cinder Fan content owns the exact symmetric five-lane contract")
	equal(int(first.ability("cinder-fan")["flux_cost"]) * 1000, int(CombatTuning.cast_definition(CombatTuning.CINDERFAN_WIRE_ID)["flux_cost"]), "Cinder Fan has an exact positive Flux decision cost")
	var expected_bursts := {
	...
	for element_id: String in AbilityCatalog.FIRST_EIGHT_ELEMENTS:
		...
		equal(int(burst.get("flux_cost", 0)) * 1000, int(CombatTuning.cast_definition(CombatTuning.CINDERFAN_WIRE_ID)["flux_cost"]), "%s Burst keeps the shared positive Flux cost" % element_id)
	...
	equal(int(first.ability("tideline")["flux_cost"]) * 1000, int(CombatTuning.cast_definition(CombatTuning.TIDELINE_WIRE_ID)["flux_cost"]), "Tideline Flux cost matches compiled behavior")
	equal(int(first.ability("tideline")["startup_ms"]), int(CombatTuning.cast_definition(CombatTuning.TIDELINE_WIRE_ID)["startup_ms"]), "Tideline startup matches compiled behavior")
	...
	equal(int(first.ability("rimewake")["flux_cost"]) * 1000, int(CombatTuning.cast_definition(CombatTuning.RIMEWAKE_WIRE_ID)["flux_cost"]), "Rimewake Flux cost matches compiled behavior")
	equal(int(first.ability("rimewake")["startup_ms"]), int(CombatTuning.cast_definition(CombatTuning.RIMEWAKE_WIRE_ID)["startup_ms"]), "Rimewake startup matches compiled behavior")
	...
	equal(int(first.ability("eclipse-disc")["flux_cost"]) * 1000, int(CombatTuning.cast_definition(CombatTuning.ECLIPSE_DISC_WIRE_ID)["flux_cost"]), "S. Wayne primary has exact positive Flux cost")
	...
	equal(int(first.ability("pocket-eclipse")["flux_cost"]) * 1000, int(CombatTuning.cast_definition(CombatTuning.POCKET_ECLIPSE_WIRE_ID)["flux_cost"]), "Pocket Eclipse Flux cost matches compiled behavior")
	equal(int(first.ability("pocket-eclipse")["startup_ms"]), int(CombatTuning.cast_definition(CombatTuning.POCKET_ECLIPSE_WIRE_ID)["startup_ms"]), "Pocket Eclipse startup matches compiled behavior")
