...
func _test_navigation_and_capture() -> void:
	...
	equal(editor.selected_action(), &"spell_layer_alt", "row navigation wraps across all spell buttons and layers")
	equal(editor.selected_device, ControlBindingEditor.DEVICE_CONTROLLER, "device navigation wraps")
	equal(editor.visible_action_indices().front(), 6, "wrapped spell selection scrolls the first visible row")
	equal(editor.visible_action_indices().back(), 17, "scrolled editor keeps the selected spell-layer row visible")
	...
	equal(editor.selected_action_index, 6, "mouse selection includes the scroll offset")
