...
func _test_validation_fails_closed() -> void:
	...
	check(SessionTransport._valid_request_packet({"sequence": 1, "action": SessionTransport.REQUEST_READY_TOGGLE}), "typed Hearth readiness request validates")
	check(SessionTransport._valid_request_packet({"sequence": 2, "action": SessionTransport.REQUEST_PRACTICE_START}), "typed Hearth start request validates")
