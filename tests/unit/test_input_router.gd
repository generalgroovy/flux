...
func _test_capture_pointer_parser() -> void:
	...
	equal(BootstrapScript.parse_capture_direction("--capture-direction=south"), Vector2i.DOWN, "movement capture accepts south")
	equal(BootstrapScript.parse_capture_direction("--capture-direction=WEST"), Vector2i.LEFT, "movement capture direction is case-insensitive")
	equal(BootstrapScript.parse_capture_direction("--capture-direction=diagonal"), Vector2i.ZERO, "movement capture rejects non-cardinal directions")
