...
const character = (definition) => freeze({ ...definition, loreStatus: "draft-placeholder", implementationStatus: "design-only", affinities: freeze(definition.affinities.map(([id, strength]) => affinity(id, strength))), activeAbilityIds: freeze([...definition.activeAbilityIds]), statBonuses: freeze({ ...definition.statBonuses }), drawbacks: freeze([...(definition.drawbacks ?? [])]), passive: freeze({ ...definition.passive }) });

export const OVERHAUL_CONTENT_STATUS = freeze({
  gameplay: "design-only",
  lore: "draft-placeholder",
  promotionGates: freeze(["mechanic-prototype", "deterministic-local-tests", "server-authority", "bot-use", "readability-accessibility", "packaged-smoke"]),
});

export const CHARACTER_ROSTER = freeze([
  character({ id: "mara", name: "Hara", raceId: "gnome", size: 2, affinities: [["light", 1], ["wind", 1]], identity: "resourceful mid-range planner", lore: "A compact strategist who always keeps a second plan.", statBonuses: { flux: 4 }, passive: { name: "SECOND PLAN", cost: 5, rule: "Once per round, swap one active at a sanctuary station." }, activeAbilityIds: ["ray", "gust-ring", "stone-shot"], ultimateAbilityId: "sun-grid", drawbacks: ["No extreme stat edge"], drawbackCredit: 4 }),
  character({ id: "nix", name: "Nico Lai", raceId: "gnome", size: 1, affinities: [["charge", 2], ["light", 1]], identity: "precision shared-device engineer", lore: "A tiny engineer whose careful devices turn one opening into three.", statBonuses: { flux: 5, speed: 2 }, passive: { name: "QUICK FIX", cost: 6, rule: "Repair one surviving personal construct after a clean dodge." }, activeAbilityIds: ["coil-hopper", "prism-tripwire", "arc-chain"], ultimateAbilityId: "small-world", drawbacks: ["Low health", "Low mass"], drawbackCredit: 9 }),
  character({ id: "samwise", name: "S. Wayne", raceId: "human", size: 3, affinities: [["dark", 2], ["light", 2]], identity: "eclipse boundary tactician", lore: "Mysterious millionaire; one suspects he earned his vast wealth through mahjong.", statBonuses: { control: 4 }, passive: { name: "BETWEEN SHADOWS", cost: 7, rule: "Crossing a personal Light/Void boundary grants one bounded reposition with a strict lockout." }, activeAbilityIds: ["prism-tripwire", "burrowed-shadow", "ray"], ultimateAbilityId: "sun-grid", drawbacks: ["Boundary-dependent routing", "No extreme body stat"], drawbackCredit: 7 }),
  character({ id: "aerwyn", name: "Spai Si", raceId: "elf", size: 2, affinities: [["wind", 2], ["light", 1], ["earth", 1]], identity: "cryptic redirect duelist", lore: "A poised duelist who lets hostile intent reveal the winning angle.", statBonuses: { speed: 4, control: 3 }, passive: { name: "THREAD THE TURN", cost: 7, rule: "A successful reflection guides one slower projectile." }, activeAbilityIds: ["gust-ring", "ray", "seed-burst"], ultimateAbilityId: "glass-moon", drawbacks: ["Fragile"], drawbackCredit: 7 }),
  character({ id: "fluup", name: "Fluup", raceId: "orc", size: 4, affinities: [["charge", 2], ["wind", 1], ["ice", 1]], identity: "storm momentum bruiser", lore: "A storm-heavy bruiser who converts every landing into another commitment.", statBonuses: { health: 4, impact: 4 }, passive: { name: "STORMWEIGHT", cost: 7, rule: "A redirected landing stores one bounded movement charge." }, activeAbilityIds: ["thunder-shove", "squall-leap", "rime-crash"], ultimateAbilityId: "bad-weather", drawbacks: ["Heavy recovery after misses"], drawbackCredit: 8 }),
  character({ id: "mog", name: "Hesus Christo", raceId: "scaleheir", size: 5, affinities: [["earth", 2], ["water", 1]], identity: "towering renewal vanguard", lore: "A Wyrm vanguard who turns broken ground into a route back into the fight.", statBonuses: { health: 7 }, passive: { name: "SECOND COMING", cost: 7, rule: "Regenerate after avoiding damage; burning suspends it." }, activeAbilityIds: ["root-rampart", "surge", "seed-burst"], ultimateAbilityId: "moss-flood", drawbacks: ["Huge target", "Slow response"], drawbackCredit: 11 }),
  character({ id: "olli", name: "Oll' I", raceId: "minotaur", size: 5, affinities: [["earth", 2], ["fire", 1], ["light", 1]], identity: "structural momentum breaker", lore: "This is one bull you don't want to grab by the horns.", statBonuses: { health: 6, impact: 5 }, passive: { name: "LABYRINTH MOMENTUM", cost: 8, rule: "Unbroken movement increases structural impact only." }, activeAbilityIds: ["sunhorn-charge", "furnace-stomp", "mirror-bulwark"], ultimateAbilityId: "burning-maze", drawbacks: ["Poor turning during commitment", "Long miss recovery"], drawbackCredit: 12 }),
  character({ id: "oh-tipi", name: "Oh Tipi", raceId: "seakin", size: 2, affinities: [["water", 2], ["ice", 1], ["charge", 1]], identity: "conductive field skirmisher", lore: "A fluid skirmisher who makes every wet route dangerous to ignore.", statBonuses: { flow: 5, control: 3 }, passive: { name: "LIVING CURRENT", cost: 7, rule: "Gain steering in personal Water without a damage bonus." }, activeAbilityIds: ["tideline", "flash-freeze", "eel-step"], ultimateAbilityId: "stormtide-basin", drawbacks: ["Field-dependent peak control"], drawbackCredit: 6 }),
  character({ id: "yrsa", name: "Ha Rekt", raceId: "scaleheir", size: 4, affinities: [["ice", 2], ["wind", 1], ["fire", 1]], identity: "aerial cold-line hunter", lore: "A Wyrm hunter who marks the escape before the enemy knows they need one.", statBonuses: { impact: 3, airControl: 4 }, passive: { name: "RIME DIVE", cost: 7, rule: "One committed aerial redirect primes a brittle landing." }, activeAbilityIds: ["rime-wing", "squall-leap", "fire-fan"], ultimateAbilityId: "sky-hook", drawbacks: ["Low FLOW capacity"], drawbackCredit: 8 }),
  character({ id: "gorum", name: "Dr. Apex", raceId: "stonewrought", size: 4, affinities: [["earth", 2], ["light", 1], ["water", 1]], identity: "armored combat medic", lore: "A combat medic from the far north.", statBonuses: { health: 6, armor: 4 }, passive: { name: "FIELD TRIAGE", cost: 7, rule: "Supporting an injured ally from personal cover grants bounded recovery, never damage." }, activeAbilityIds: ["root-rampart", "spring", "mirror-bulwark"], ultimateAbilityId: "deep-spring", drawbacks: ["Slow movement", "Large target"], drawbackCredit: 10 }),
  character({ id: "treevor", name: "Treevor the Mason", raceId: "rootwarden", size: 5, affinities: [["earth", 2], ["wind", 1], ["fire", 1]], identity: "mud-and-herb terrain mason", lore: "Known for making the finest mudpies ye ever done seen; he also lika da herb.", statBonuses: { health: 7, reach: 4 }, passive: { name: "DEEP ROOTS", cost: 8, rule: "Personal ground grants stability; sustained Fire remains dangerous." }, activeAbilityIds: ["root-rampart", "branch-gale", "ember-seed"], ultimateAbilityId: "crown-wildfire", drawbacks: ["Huge target", "Fire exposure"], drawbackCredit: 13 }),
  character({ id: "vey", name: "Wa Bidi", raceId: "sylph", size: 1, affinities: [["wind", 2], ["charge", 1]], identity: "battlecry air-route specialist", lore: "Her battlecry \"WABIDI BIBIDI!\" is often the last thing her poor victims hear.", statBonuses: { speed: 6, airControl: 5 }, passive: { name: "THIN AIR", cost: 7, rule: "One extra air redirect with sharply reduced impact resistance." }, activeAbilityIds: ["squall-leap", "eel-step", "gust-ring"], ultimateAbilityId: "sky-hook", drawbacks: ["Very fragile", "High knockback"], drawbackCredit: 12 }),
  character({ id: "rote-baron", name: "The Red Baron", raceId: "undead", size: 3, affinities: [["dark", 2], ["fire", 1], ["ice", 1]], identity: "airborne formation controller", lore: "An undead officer who turns dangerous air lanes into a disciplined formation.", statBonuses: { control: 4 }, passive: { name: "COLD ASHES", cost: 7, rule: "Expired personal Fire leaves a brief harmless chilled remnant." }, activeAbilityIds: ["crimson-comet", "night-flak", "rime-wing"], ultimateAbilityId: "the-dead-sky", drawbacks: ["Reduced conventional healing"], drawbackCredit: 7 }),
  character({ id: "steezo", name: "Steezo", raceId: "goblin", size: 1, affinities: [["fire", 1], ["charge", 2], ["light", 1]], identity: "volatile combo engineer", lore: "Sheeeeeeeeeeeeeeee.", statBonuses: { flux: 4, speed: 3 }, passive: { name: "QUESTIONABLE ENGINEERING", cost: 7, rule: "Opponent-destroyed constructs refund bounded Flux; self-detonation does not." }, activeAbilityIds: ["spark-keg", "prism-tripwire", "coil-hopper"], ultimateAbilityId: "safe-machine", drawbacks: ["Fragile", "Construct-dependent"], drawbackCredit: 10 }),
  character({ id: "luma", name: "Hidn Leef", raceId: "rootwarden", size: 4, affinities: [["water", 2], ["earth", 1], ["light", 1]], identity: "concealed grove field support", lore: "A hidden Treefolk support who lets the battlefield bloom around an unseen route.", statBonuses: { flux: 4, control: 3 }, passive: { name: "AFTERBLOOM", cost: 7, rule: "A clean personal reaction strengthens the next support field, never direct damage." }, activeAbilityIds: ["spring", "seed-burst", "ray"], ultimateAbilityId: "bloom", drawbacks: ["Slow repositioning", "Fire-vulnerable growth"], drawbackCredit: 9 }),
  character({ id: "brum", name: "Grimm Bow", raceId: "troll", size: 4, affinities: [["dark", 2], ["earth", 1], ["water", 1]], identity: "void-current terrain archer", lore: "Draft placeholder; final character description pending author rewrite.", statBonuses: { health: 3, control: 4 }, passive: { name: "DROWNED MARK", cost: 7, rule: "Hitting a target displaced by personal Void or Water steadies the next Stone Shot, never increasing damage." }, activeAbilityIds: ["stone-shot", "void-pull", "tideline"], ultimateAbilityId: "moss-flood", drawbacks: ["Large target", "Slow recovery after a missed shot"], drawbackCredit: 9 }),
]);
...
export function validateOverhaulContent() {
  ...
  if (CHARACTER_ROSTER.length !== 16) errors.push("exactly sixteen future characters are required");
  const requiredCharacterNames = ["Oh Tipi", "S. Wayne", "The Red Baron", "Steezo", "Treevor the Mason", "Oll' I", "Fluup", "Wa Bidi", "Nico Lai", "Spai Si", "Hidn Leef", "Ha Rekt", "Dr. Apex", "Hara", "Hesus Christo", "Grimm Bow"];
  const characterNames = new Set(CHARACTER_ROSTER.map((entry) => entry.name));
  for (const name of requiredCharacterNames) if (!characterNames.has(name)) errors.push(`missing required future character ${name}`);
  ...
  for (const characterEntry of CHARACTER_ROSTER) {
    ...
    if (typeof characterEntry.lore !== "string" || characterEntry.lore.trim().length === 0) errors.push(`${characterEntry.id} needs character lore`);
    if (characterEntry.loreStatus !== OVERHAUL_CONTENT_STATUS.lore) errors.push(`${characterEntry.id} lore must remain draft placeholder copy`);
    if (characterEntry.implementationStatus !== OVERHAUL_CONTENT_STATUS.gameplay) errors.push(`${characterEntry.id} must remain design-only`);
  ...
  const requiredNames = ["COLD ASHES", "CRIMSON COMET", "NIGHT FLAK", "RIME WING", "THE DEAD SKY", "DEEP ROOTS", "ROOT RAMPART", "BRANCH GALE", "EMBER SEED", "CROWN OF THE WILDFIRE", "BETWEEN SHADOWS", "PRISM TRIPWIRE", "BURROWED SHADOW", "Ray", "Sun Grid", "QUESTIONABLE ENGINEERING", "SPARK KEG", "COIL HOPPER", "PERFECTLY SAFE MACHINE", "LIVING CURRENT", "TIDELINE", "FLASH FREEZE", "EEL STEP", "STORMTIDE BASIN", "LABYRINTH MOMENTUM", "SUNHORN CHARGE", "FURNACE STOMP", "MIRROR BULWARK", "THE BURNING MAZE", "STORMWEIGHT", "THUNDER SHOVE", "SQUALL LEAP", "RIME CRASH", "BAD WEATHER"];
