...
| 2 | Comfortable in-game controls | Implemented: schema 6 and the walk-up lectern capture keyboard/mouse-button/wheel/controller inputs including spell slots 1–5, swap conflicts, unbind/reset/cancel, save immediately and suppress local play safely while open. |
| 3 | Five-slot spell grammar | In progress: protocol 22 carries stable `spell_1`…`spell_5` edges, schema-v2 loadouts validate five unique ordered entries, the HUD shows current/empty slots and the two proven kit actions work through slots 1/2; next add Loom editing plus truthful shape metadata and projectile/beam/spray/field/movement-defense fixtures. |
