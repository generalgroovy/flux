...
| C4 first-eight spell matrix — complete | Instantiate Fire/Water/Earth/Wind/Charge/Ice/Light/Dark rows across Bolt/Burst/Spray/Beam/Field columns with stable IDs, positive costs, shared family resolvers, element presentation and global Loom availability. | Every champion can weave every one of the 40 matrix cells; all release at 120 Hz, row/column completeness fails closed, Burst comparisons remain mechanically identical, and Vector Lance stays an explicit extra variant. |
...
| Composition | Runtime composes body, receiving-surface shadow, status/aura, cast effect, projectile and environment as separately editable presentation layers; every body renders at `1.00×` around the shared feet pivot and authored atlas silhouettes carry the small/middle/large distinction; cast startup is anchored to a forward hand lane, and `atlas_row` is authored per champion in the visual recipe. |
| Shared style | The Red Baron's darkest visible material clusters deterministically seed a restrained one-pixel exterior ink for all foundation frames; the pass is cell-bounded and may not recolor, rescale, merge or bleed identities. |
...
|---:|---|---|
| D0 — complete | One validated direction resolver and fixed direction order now serve travel, facing, hand origins, effects and captures. | Boundary, zero-vector, hysteresis and repeated 120 Hz tests resolve identically without touching simulation vectors; the four-cardinal body fallback remains explicit. |
| D1 — complete | Keyboard, controller, prediction, replay and Farflow movement now share all eight command sectors; digital diagonals are normalized, controller magnitude survives quantization, and radial acceleration removes diagonal startup advantage. | Equal authored heading speed/cost/collision behavior in every direction at 120 Hz; all components survive prediction/replay/real ENet loopback without drift, under protocol 32. |
...
| D4 — complete | Added reviewed native diagonal jump/slide/roll art for both foundation champions, retained the presentation-only invulnerability cues, and added opposite walk/sprint contacts in every direction. | Every current evasion state and both locomotion contacts preserve direction, shadow and pivot without changing simulation; fail-closed cells and 120 Hz cadence tests are green. |
...
2. Fixed-tick unit fixtures at the authoritative 120 Hz tick.
3. Simulation integration: cost, cooldown, collision, cover, reset, overflow.
4. Network serialization and hostile-input refusal.
5. Windows source boots at 120 Hz, then imported-resource boots.
